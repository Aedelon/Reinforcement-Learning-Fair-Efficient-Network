...
class PPOPolicyNetwork:
    def __init__(self, num_features, layer_size, num_actions, epsilon=.2, learning_rate=9e-4):
        self.tf_graph = tf.Graph()
...
def main_loop(name, n_episode=100000, max_steps=1000, epsilon=0.2, controler_layer_size=128, sub_policy_layer_size=256):
    ...
    meta_Pi = PPOPolicyNetwork(num_features=15, num_actions=n_signal, layer_size=controler_layer_size, epsilon=epsilon,
    ...
    for i in range(n_signal):
        Pi.append(PPOPolicyNetwork(num_features=13, num_actions=n_actions, layer_size=sub_policy_layer_size,
    ...
    while i_episode < n_episode:
        ...
        while steps < max_steps:
            ...
            if steps % T == 0:
                for i in range(n_agent):
                    meta_rewards[i].append(utili[i] / (0.1 + rat[i]))
